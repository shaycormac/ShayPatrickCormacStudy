...
import com.assassin.gsonstudy.entity.SimpleResponse;
 ...
 * @Ltd: GoldMantis
 * @Date: 2017/4/27 15:15
 * @Version: 1.0
 * @Description: 对okhttp响应的数据进行解析。
 */

public abstract class GsonUtil<T>
{
    
    public  <T> T parseToJson(Response response) throws NoSuchMethodException {
        
        Type genType = getClass().getGenericSuperclass();
        //从上述的类中取出真实的泛型参数，有些类可能有多个泛型，所以是数值
        Type[] params = ((ParameterizedType) genType).getActualTypeArguments();
        //我们的示例代码中，只有一个泛型，所以取出第一个，得到如下结果
        //com.lzy.demo.model.JsonResponse<com.lzy.demo.model.ServerModel>
        Type type = params[0];
        ...
        //class com.lzy.demo.model.JsonResponse
        ...
        if (typeArgument == Void.class) {
            //无数据类型,表示没有data数据的情况（以  new DialogCallback<JsonResponse<Void>>(this)  以这种形式传递的泛型)
        ...
        {
            //有数据类型，表示有data
            JsonResponse lzyResponse = Convert.fromJson(jsonReader, type);
            response.close();
